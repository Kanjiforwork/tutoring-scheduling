...
export const editSchema = z.object({ ...fields, mode: z.enum(['one_to_one','pair']).optional(), bookings: z.array(z.object({ id: z.string().min(1).optional(), replacementStudentId: z.string().min(1).optional(), studentId: z.string().min(1), status: z.enum(['booked','cancelled','no_show']) }).strict()).min(1).max(100).optional(), expectedVersion:z.number().int().positive(), reason:z.string().trim().min(1,'A reason is required').max(500) }).strict();

...
export function presentConflict(warning: Warning, sessions: Session[], candidateId?: string): ConflictPresentation {
  ...
  switch (warning.code) {
    case 'STUDENT_OVERLAP': return {title: `${student?.studentName ?? 'This student'} has overlapping lessons`, action: 'Choose a different time.', slots};
    case 'TUTOR_OVERLAP': return {title: `${first?.tutorName ?? 'Tutor'} is already teaching`, action: 'Choose another tutor or time.', slots};
    case 'ROOM_OVERLAP': return {title: `${first ? `Room ${first.roomId}` : 'This room'} is already booked`, action: 'Choose another room or time.', slots};
    ...
    case 'TUTOR_DAILY_LIMIT': {
      if (!first) return {title: 'Tutor booking limit exceeded', action: 'Choose another tutor or day.', slots: []};
    ...
    default: return {title: 'This session needs attention', action: 'Review the schedule before saving.', slots: []};
  }
}

...
function Warnings({ warnings, prominent = false, sessions = [] }: { warnings: Warning[]; prominent?: boolean; sessions?: Session[] }) {
  if (prominent && warnings.length) return <section className="session-conflict-alert" role="alert" aria-label="Existing schedule conflicts"><div className="session-conflict-heading"><AlertTriangle size={18} aria-hidden="true" /><strong>Schedule conflict</strong><span className="conflict-saved-label">Saved schedule</span></div>{warnings.map((warning, index) => {
    ...
    const overlap = ['STUDENT_OVERLAP','TUTOR_OVERLAP','ROOM_OVERLAP'].includes(warning.code) && related.length > 1;
    const title = warning.code === 'STUDENT_OVERLAP' ? `${student?.studentName ?? 'Student'} has two lessons at once` : warning.code === 'TUTOR_OVERLAP' ? `${related[0]?.tutorName} is teaching two lessons at once` : `${related[0]?.roomId} is booked for two lessons`;
    return <div className="conflict-item" key={`${warning.code}-${index}`}>{overlap ? <><p className="conflict-title">{title}</p><div className="conflict-sessions">{related.map(s => <div className="conflict-session" key={s.id}><span>{shortDate(s.date)}</span><strong>{s.startTime}–{endTime(s)}</strong><span>Room {s.roomId}</span></div>)}</div></> : <p className="conflict-title">{warning.message.replace(/\d{4}-\d{2}-\d{2}/g, shortDate)}</p>}</div>;
  })}</section>;
  return warnings.length ? <details className="warning-details"><summary><AlertTriangle size={14} /> {warnings.length === 1 ? warnings[0].code.toLowerCase().replaceAll('_', ' ') : `${warnings.length} existing issues`}<ChevronDown size={12} /></summary><div className="warning-copy">{warnings.map((w, i) => <p key={i}>{w.message}</p>)}</div></details> : <span className="no-issues">—</span>;
}

...
export function SessionSelect({ label, value, options, onChange, placeholder = 'Select an option', required = false, editField, availability, disabled = false }: {
  label: string; value: string; options: SelectOption[]; onChange: (value: string) => void;
  placeholder?: string; required?: boolean; editField?: string; availability?: string; disabled?: boolean;
}) {
  const id = useId();
  ...
  return <span className="session-select">
    <button ref={trigger} disabled={disabled} type="button" role="combobox" aria-label={label} aria-expanded={open} aria-controls={id} aria-haspopup="listbox" aria-required={required} aria-invalid={missing || !!selected?.hint} aria-describedby={hint ? `${id}-hint` : undefined} data-edit-field={editField}
